...
def main(arguments: list[str] | None = None) -> int:
    ...
    configure_vm(vboxmanage, parsed.name, parsed.memory, parsed.cpus, windows_path(iso_path))
    run([vboxmanage, "startvm", parsed.name, "--type", "headless" if parsed.headless else "gui"])
    return 0
